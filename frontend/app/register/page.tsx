    ...
    try {
      if (role === 1) {
        ...
        await userRegisterService({
          ...data,
          userType: role === 2 ? userRoleType.COMEDIAN : userRoleType.VIEWER,
        });
      }

      toast.success(
        `${
          role === 1 ? "Admin" : role === 2 ? "Comedian" : "Viewer"
      ...
      <div className="register-here w-[30%] p-5 min-h-[70%] bg-white rounded-lg">
        ...
        <div className="w-[100%] mt-5 flex items-center justify-center">
          <div className=" bg-black flex w-[90%] text-white  my-5 justify-around p-1 gap-4">
            <p
              className={`${
                role === 1 && "bg-white text-black"
              } px-3 text-md transition-all cursor-pointer `}
              onClick={() => setRole(1)}
            >
              Admin
            </p>
        ...
        <form
          ...
          className="flex flex-col justify-start gap-5 mt-5"
        >
          <Input
            inputType="text"

...
import { adminSignInService, signInUserService } from '../Services/auth.service';
import { useRouter } from 'next/navigation';

export type SignInSchemaType = z.infer <typeof SignInSchema>
function page() {

   const {register , handleSubmit , formState : {errors} , reset} = useForm<SignInSchemaType>({
  ...
  const submitFormData = async(data : SignInSchemaType) =>{
        setIsLoading(true)
       try {
             console.log("Print daya " , data)
              const {isAdmin , ...rest} = data;
              if (isAdmin) {

                  await adminSignInService(rest);
              }
              else {
                  await signInUserService(rest);
              }
  ...
  return ( 
    <div className='bg-gray-200 w-full h-screen text-black flex items-center justify-center'>
      ...
      <div className="register-here w-[30%] p-5 min-h-[70%] bg-white rounded-lg">
            ...
            <form onSubmit={handleSubmit(submitFormData)} className=' mt-12 flex flex-col justify-start gap-5' >
            
             <Input inputType='text' placeHolder='Enter your Email' register={register("email")} error={errors.email?.message} />
             <Input inputType='password' placeHolder='Enter your Password' register={register("password")} error={errors.password?.message}/>

             <div className=' mx-8 flex items-center justify-start gap-4'>
              <input type="checkbox" id='is-admin' {...register("isAdmin")}  />
              <label htmlFor="is-admin">Admin</label>   
              </div>  

             <DarkButton  btnText={isLoading ? "logining..." : "login"} type='submit' />
